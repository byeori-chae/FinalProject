...
public class BoardDAO {
	...
	private static BoardDAO instance; //멤버필드 변수 선언
	...
	public int getListCount(String items, String text) {
		...
		if (items == null && text == null)
			sql = "SELECT count(*) from tbl_board"; //count: 레코드 갯수 출력  
		else
			sql = "SELECT count(*) FROM tbl_board where " + items + " like '%" + text + "%'";
	...
	public ArrayList<BoardDTO> getBoardList(int page, int limit, String items, String text) {
		...
		if (items == null && text == null)
			sql = "select * from tbl_board ORDER BY num DESC";
		else
			sql = "SELECT * FROM tbl_board where " + items + " like '%" + text + "%' ORDER BY num DESC ";
	...
	public void updateHit(int num) {
		...
		try {
			...
			String sql = "select hit from tbl_board where num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			int hit = 0;
			if (rs.next())
				hit = rs.getInt("hit") + 1;
			sql = "update tbl_board set hit=? where num=?";
			pstmt = conn.prepareStatement(sql);		
			pstmt.setInt(1, hit);
			pstmt.setInt(2, num);
			pstmt.executeUpdate();
	...
	public void updateBoard(BoardDTO board) {
		...
		try {
			String sql = "update tbl_board set name=?, subject=?, content=? where num=?";
